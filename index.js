...
var index_default = {
  async fetch(request, env) {
    ...
    let path = url.pathname.replace(/^\/functions\/v1\/streamline/, "").replace(/^\/streamline/, "") || "/";
    ...
    if (path === "/health") return new Response(JSON.stringify({ ok: true, version: 21, stripe: "live", status: "LIVE" }), { headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } });
    if (path === "/chat" || path === "/submit") {
      const target = `https://huvfgenbcaiicatvtxak.supabase.co/functions/v1/streamline${path}${url.search}`;
      const h = new Headers(request.headers);
      h.delete("host");
      const pr = await fetch(target, { method: request.method, headers: h, body: ["GET", "HEAD"].includes(request.method) ? void 0 : request.body, redirect: "follow" });
      const rh = new Headers(pr.headers);
      rh.set("Access-Control-Allow-Origin", "*");
      return new Response(pr.body, { status: pr.status, headers: rh });
    }
    return new Response(HTML, { status: 200, headers: { "Content-Type": "text/html; charset=utf-8", "Cache-Control": "no-cache, no-store, must-revalidate", "X-Streamline-Version": "21" } });
  }
};
var HTML = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>Streamline \u2014 Submit an idea. We build it. You earn forever.</title>
...
<div class="topbar">
  <span><span class="dot"></span>11 apps live</span>
  <span class="sep">\xB7</span><span>$12.7k MRR</span>
  <span class="sep">\xB7</span><span>20\u201330% commission, forever</span>
  <span class="sep">\xB7</span><span>$3,677 paid to submitters</span>
</div>
...
<main>
  <section class="hero">
    ...
    <div class="hero-stats">
      <div class="stat"><div class="stat-val">11</div><div class="stat-lbl">Apps live</div></div>
      <div class="stat"><div class="stat-val">$12.7k</div><div class="stat-lbl">Monthly revenue</div></div>
      <div class="stat"><div class="stat-val">30%</div><div class="stat-lbl">Max commission</div></div>
      <div class="stat"><div class="stat-val">$3,677</div><div class="stat-lbl">Paid out</div></div>
    </div>
  </section>
  ...
  <section>
    ...
    <div class="feat-grid">
      <div class="feat-card main">
        <div class="feat-tag live">\u25CF FLAGSHIP \xB7 LIVE</div>
        <h3>LessonLab</h3>
        ...
        <div class="feat-chips">
          <span class="feat-chip">$3,200 / mo</span>
          <span class="feat-chip">\u{1F1E6}\u{1F1FA} Australia</span>
          <span class="feat-chip">iOS + Web</span>
        </div>
      </div>
      <div class="feat-card">
        ...
        <div class="feat-chips">
          <span class="feat-chip">Voice input</span>
          <span class="feat-chip">OCR scanning</span>
          <span class="feat-chip">Q2 2026</span>
        </div>
      </div>
    </div>
  </section>
  ...
  <section id="submit" style="padding-top:0">
    <div class="form-outer">
      <div class="form-card">
        ...
        <div class="sel-tier-bar" id="sel-tier"><span>Selected: <strong>Priority</strong></span><span>$99 \xB7 25% commission</span></div>
...
<footer>
  <div class="foot-inner">
    ...
    <p>Built in Melbourne by Paddy Gallivan \xB7 Part of the Asgard ecosystem \xB7 v21 \xB7 \xA9 2026</p>
  </div>
</footer>
...
<div class="chat-panel" id="chat-panel">
  <div class="chat-head">
    ...
    <button class="chat-close" onclick="cc()">\xD7</button>
  </div>
  <div class="chat-body" id="chat-body">
    <div class="msg ai">Hey! Ask me about submitting ideas, how commissions work, or what's on the board. \u{1F44B}</div>
  </div>
...
<script>
var IDEAS=[{id:1,title:"LessonLab",desc:"931 PE lessons, curriculum-aligned.",cat:"Education",rev:3200,status:"live",votes:127,emoji:"\u{1F4DA}"},{id:2,title:"PE Lesson Timer",desc:"Bulletproof timer for PE classes.",cat:"Sport",rev:890,status:"live",votes:94,emoji:"\u23F1"},{id:3,title:"Footy Tips",desc:"AFL tipping for friends and family.",cat:"Sport",rev:450,status:"live",votes:88,emoji:"\u{1F3C6}"},{id:4,title:"Trivia Host Pro",desc:"Everything a pub trivia host needs.",cat:"Entertainment",rev:1200,status:"live",votes:76,emoji:"\u{1F3A4}"},{id:5,title:"Superleague Fantasy",desc:"Fantasy AFL for die-hards.",cat:"Sport",rev:680,status:"live",votes:71,emoji:"\u26A1"},{id:6,title:"School Sport Portal",desc:"Victoria interschool competitions.",cat:"Education",rev:1400,status:"live",votes:65,emoji:"\u{1F3EB}"},{id:7,title:"KBT Trivia",desc:"Live trivia events platform.",cat:"Entertainment",rev:2100,status:"live",votes:53,emoji:"\u{1F9E0}"},{id:8,title:"Family Comp",desc:"AFL tipping, racing comps, payouts.",cat:"Sport",rev:0,status:"building",votes:59,emoji:"\u{1F389}"},{id:9,title:"Judy's Kitchen",desc:"Save and share family recipes.",cat:"Food",rev:0,status:"queued",votes:48,emoji:"\u{1F373}"},{id:10,title:"Carnival Live",desc:"Real-time carnival scoreboard.",cat:"Education",rev:320,status:"live",votes:47,emoji:"\u{1F4CA}"},{id:11,title:"PE Assessment",desc:"Track students against PE curriculum.",cat:"Education",rev:580,status:"live",votes:42,emoji:"\u{1F4CB}"},{id:12,title:"Pub Quiz Buzzer",desc:"Team buzzers for live trivia nights.",cat:"Entertainment",rev:0,status:"queued",votes:31,emoji:"\u{1F3AF}"},{id:13,title:"Gym Buddy",desc:"Match local training partners.",cat:"Health",rev:0,status:"queued",votes:28,emoji:"\u{1F4AA}"},{id:14,title:"Allergen Scanner",desc:"Scan food labels, highlight allergens.",cat:"Health",rev:0,status:"queued",votes:17,emoji:"\u{1F9EA}"}];
var hist=[],busy=false,selTier="Priority",selPrice=99,selPct=25;
function ri(){var g=document.getElementById("ideas"),s=IDEAS.slice().sort(function(a,b){return b.votes-a.votes}),h="";var voted=JSON.parse(localStorage.getItem("slv")||"[]");for(var i=0;i<s.length;i++){var x=s[i],bc=x.status==="live"?"b-live":x.status==="building"?"b-building":"b-queued",bt=x.status==="live"?"\u25CF Live":x.status==="building"?"\u25D0 Building":"\u25CB Queued",rv=x.rev>0?'<span class="badge b-rev">$'+x.rev.toLocaleString()+'/mo</span>':"";var iv=voted.indexOf(x.id)!==-1;h+='<div class="idea" data-cat="'+x.cat+'"><div class="idea-top"><div class="idea-icon">'+x.emoji+'</div><button class="vote-btn'+(iv?" voted":"")+'" onclick="vt('+x.id+')" id="vb-'+x.id+'"><span class="vote-arr">\u25B2</span><span class="vote-cnt" id="vc-'+x.id+'">'+x.votes+'</span></button></div><h4>'+x.title+'</h4><p class="idea-desc">'+x.desc+'</p><div class="idea-foot"><span class="badge '+bc+'">'+bt+'</span><span class="badge b-cat">'+x.cat+'</span>'+rv+'</div></div>';}g.innerHTML=h;}
function vt(id){try{var v=JSON.parse(localStorage.getItem("slv")||"[]");if(v.indexOf(id)!==-1)return;document.getElementById("vb-"+id).classList.add("voted");var c=document.getElementById("vc-"+id);c.textContent=parseInt(c.textContent)+1;v.push(id);localStorage.setItem("slv",JSON.stringify(v));}catch(e){}}
function setTier(n,p,pct){selTier=n;selPrice=p;selPct=pct;document.getElementById("sel-tier").innerHTML='<span>Selected: <strong>'+n+'</strong></span><span>$'+p+' \xB7 '+pct+'% commission</span>';}
document.addEventListener("click",function(e){var t=e.target.closest(".tier");if(t&&t.dataset.tier){var m={Standard:[29,20],Priority:[99,25],Equity:[299,30]};setTier(t.dataset.tier,m[t.dataset.tier][0],m[t.dataset.tier][1]);document.getElementById("submit").scrollIntoView({behavior:"smooth"});}var ch=e.target.closest(".chip");if(ch){var c=ch.getAttribute("data-cat");document.querySelectorAll(".chip").forEach(function(x){x.classList.remove("active")});ch.classList.add("active");document.querySelectorAll(".idea").forEach(function(x){x.style.display=c==="All"||x.getAttribute("data-cat")===c?"":"none";});}});
...
function submitIdea(){
  ...
  fetch("/submit",{method:"POST",headers:{"Content-Type":"application/json"},
    body:JSON.stringify({title:title,name:name,email:email,category:cat,description:desc,tier:selTier})})
  .then(function(r){return r.json()})
  .then(function(d){
    if(d.success){
      window.location.href=STRIPE[selTier]+"?prefilled_email="+encodeURIComponent(email);
    }else{
